...
class Message(Base, Identifiable):
    ...
    MainData = PydanticModel.column_model(text, user_id, created_at, updated_at, message_id=id)
    ...
    @classmethod
    def get_history(cls, session: sessionmaker, limit: int = 20, offest: int = 0) -> List[Message] | None:
        return cls.get_history_with_filter(
            session,
            select(cls).filter_by(deleted=None),
            limit,
            offest
        )
